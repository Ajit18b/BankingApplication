...
const AdminControlsAccountCreation = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (error.response) {
        ...
        setResponseMessage(data.message || `Error: ${status} Email and account number already registered`);
        setIsError(true);
        ...
        // Handle cases where an unexpected error occurs
  ...
  const generatePDF = (data) => {
    ...
    doc.text("This is to certify that the following account has been successfully created:", 20, 40);
    ...
    doc.text(`Issued on: ${formattedDateTime}`, 20, startY + (fields.length + 1) * rowHeight + 10);
